...
| 路径 | 用途 |
|---|---|
| `server/build-selfbuild.sh` | 本地：交叉编译单二进制，断言 CLI 行为 + 软浮点 + 静态 |
| `server/make-artifact.sh` | VPS：打成 `tar.gz` + sha256，支持三种来源 |
| `server/nginx-ts.conf.example` | VPS：443 + 随机路径托管工件 |
| `router/ts.conf` | 路由器：唯一配置点（工件 URL/sha、headscale 参数、tmpfs 上限） |
| `router/ts-fetch` | 下载 → 校验 → 解包 → 自检；`--with-cli` 时才拉 CLI |
| `router/ts-login` | 首次注册：拉 daemon + 临时 CLI → `tailscale up` → 删 CLI 释放内存 |
| `router/tailscale-ram` | procd 服务：开机拉取、守护 daemon、按需 `tailscale up` |
| `router/install.sh` | 一键落地 |
...
`SOURCE=local` 会再校验一次软浮点/静态才打包，然后打印要粘进 `ts.conf` 的那一行。
...
默认走官方静态构建，会自动校验官方 `.sha256` sidecar。输出形如：

```
   <sha-daemon> https://.../tailscaled_1.102.4_mipsle.tar.gz      # TS_ARTIFACTS
   <sha-cli>    https://.../tailscale-cli_1.102.4_mipsle.tar.gz   # TS_CLI_ARTIFACTS
```
...
- **用自编译单二进制**：只需要填 `TS_ARTIFACTS` 一行（`TS_CLI_ARTIFACTS` 留空 —— CLI 就在同一个文件里，`ts-fetch` 会软链出 `tailscale`）。
- **用官方静态包**：`make-artifact.sh` 会打印两行，第一行填 `TS_ARTIFACTS`（daemon），第二行填 `TS_CLI_ARTIFACTS`（CLI）。

再把 `LOGIN_SERVER` / `ADVERTISE_ROUTES` 改成你自己的。

```sh
echo -n '<preauthkey>' > /etc/tailscale/authkey
...
它依次做：拉 daemon → 确保有 CLI → 启动服务 → `tailscale up --login-server=... --authkey=... --advertise-routes=...` →（官方包路径）**成功后删掉 CLI**（省 32MB）。失败时保留 CLI 便于重试。

之后每次开机只走 `ts-fetch`：自编译单二进制重下 9.9MB，官方包只下 17.7MB 的 daemon。都不再需要单独的 CLI —— 因为 `tailscaled.state` 里已经存了 prefs，daemon 启动会自行恢复连接。

> **首装是内存最紧张的时刻**（daemon 38.7 + CLI 解包 32.1 + CLI 包 14.3 ≈ 85MB，加上系统约 120MB）。
...
```sh
ts-fetch --with-cli && /tmp/tailscale/tailscale --socket=/tmp/tailscale/tailscaled.sock status
rm -f /tmp/tailscale/tailscale      # 看完删掉
```
...
| sha256 不匹配 | 重新跑 `make-artifact.sh` 并把新 sha 同步进 `ts.conf` |
